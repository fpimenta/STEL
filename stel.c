...
int parse_input3(int argc, char** argv, struct simulacao *simulacao_atual, struct simulacao *simulacao_especial){
    extern char *optarg;
    extern int optind, opterr, optopt;
    int args_obrigatorios = 4, c;

    while ((c = getopt (argc, argv, "s:r:w:o:vch")) != -1)
    switch (c) {
        case 'h':
            printf("\n"UNDERLI"\tDiscrete Event Traffic Simulation - STEL 2018/2019 @ FEUP"END_UND);
            printf("\n\nUsage: %s\t {-s NrOfSamples} {-r NrOfResources} {-w WaitingListLength} {-o SpecialOperators}[-v] [-c]\n"
                   "\n\t-v\tMakes the program verbose\n"
		   "\t-c\tSeed the PRNG with the current system time\n\n", argv[0]);
            return -1;
        case 's':
        ...
        case 'o':
            args_obrigatorios--;
            (*simulacao_especial).nr_recursos = atoi(optarg);
            break;
        case '?':
...
double gerarChamada(struct simulacao *simulacao_atual, double ultima_chegada, double *delay, double * hist_delay, double * hist_previsao, struct simulacao *simulacao_especial){
    ...
    double tempo_recurso_libertado = 0, tempo_chegada_recurso = 0;
    ...
    while (partida_a_libertar < tempo_atual){
        ...
        tempo_chegada_recurso = ((lista*)(*simulacao_atual).lista_recursos)->chegada_sistema;
        if (((lista*)(*simulacao_atual).lista_recursos)->tipo == PARTIDA_ESPECIALIZADA)
            gerarChamadaEspecial(simulacao_especial, tempo_recurso_libertado, tempo_chegada_recurso);
        ...
        if ((*simulacao_atual).lista_espera_ocupada){
            ...
            if((((lista*)(*simulacao_atual).lista_espera)->espera_estimada - this_delay) > (*simulacao_atual).pior_previsao)(*simulacao_atual).pior_previsao2 = ((lista*)(*simulacao_atual).lista_espera)->espera_estimada - this_delay; 
            ...
            if (tipo_chamada == CHEGADA_GERAL){
                ...
                (*simulacao_atual).lista_recursos = adicionar((*simulacao_atual).lista_recursos, PARTIDA_GERAL, tempo_recurso_libertado+d,tempo_recurso_libertado, tempo_inicial);
                average(d);
            } else {
                double d = gaussian();
                (*simulacao_atual).lista_recursos = adicionar((*simulacao_atual).lista_recursos, PARTIDA_ESPECIALIZADA, tempo_recurso_libertado+d,tempo_recurso_libertado, tempo_inicial);
                average(d);
            }
    ...
    if ((*simulacao_atual).recursos_ocupados >= (*simulacao_atual).nr_recursos){
        ...
        if ((*simulacao_atual).lista_espera_ocupada < (*simulacao_atual).tamanho_espera){
            ...
            (*simulacao_atual).lista_espera = adicionar((*simulacao_atual).lista_espera, CHEGADA, tempo_atual, estimador, tempo_atual);
        } else {
        ...
        if (typeOfCall() == CHEGADA_GERAL){
            double d = exponential(EXPONENT_AVG,EXPONENT_MIN,EXPONENT_MAX);
            (*simulacao_atual).lista_recursos = adicionar((*simulacao_atual).lista_recursos, PARTIDA_GERAL, tempo_atual+d,tempo_atual,tempo_atual);
            average(d);
        } else {
            double d = gaussian();
           
            (*simulacao_atual).lista_recursos = adicionar((*simulacao_atual).lista_recursos, PARTIDA_ESPECIALIZADA, tempo_atual+d,tempo_atual, tempo_atual);
            average(d);
        }
    }
...
double gerarChamadaEspecial(struct simulacao *simulacao_atual, double ultima_chegada, double chegada_recurso){
    ...
    double tempo_atual = ultima_chegada;
    ...
    while (partida_a_libertar < tempo_atual){
        ...
        if ((*simulacao_atual).lista_espera_ocupada){
            // Existindo evento na lista de espera, processar esse
            double tempo_inicial = ((lista*)(*simulacao_atual).lista_espera)->chegada_sistema;
            //double this_delay = tempo_recurso_libertado - tempo_inicial;
            (*simulacao_atual).lista_espera = remover((*simulacao_atual).lista_espera);
            double d = exponential(EXPONENT_AVG_SPEC,EXPONENT_MIN_SPEC,-1);
            //printf("DELAY2: %f\n", tempo_recurso_libertado-tempo_inicial);
            average2(tempo_recurso_libertado-tempo_inicial);
            (*simulacao_atual).lista_recursos = adicionar((*simulacao_atual).lista_recursos, PARTIDA_GERAL, tempo_recurso_libertado+d,tempo_recurso_libertado, tempo_inicial);
    ...
    if ((*simulacao_atual).recursos_ocupados >= (*simulacao_atual).nr_recursos){
        ...
        (*simulacao_atual).lista_espera = adicionar((*simulacao_atual).lista_espera, CHEGADA, tempo_atual, tempo_atual, chegada_recurso);
    } else {
        // Adicionar chegada, ocupar recurso, gerar partida
        (*simulacao_atual).recursos_ocupados++;
        //printf("DELAY1: %f\n", tempo_atual-chegada_recurso);
        average2(tempo_atual-chegada_recurso);
        double d = exponential(EXPONENT_AVG_SPEC,EXPONENT_MIN_SPEC,-1);
        (*simulacao_atual).lista_recursos = adicionar((*simulacao_atual).lista_recursos, PARTIDA_GERAL, tempo_atual+d,tempo_atual, chegada_recurso);
    }
